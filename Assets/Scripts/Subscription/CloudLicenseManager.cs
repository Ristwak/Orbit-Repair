...
public class CloudLicenseManager : MonoBehaviour
{
    ...
    public Transform vrCamera;   // Assign your VR or Main Camera here in Inspector (Can be left empty for static panel)
    ...
    {
        if (subscriptionPanel == null) return;

        subscriptionPanel.SetActive(true);

        // If camera is assigned, position panel in front of camera
        if (vrCamera != null)
        {
        ...
        {
            // If no camera is assigned, place the panel at a static position (e.g., fixed above)
            subscriptionPanel.transform.position = new Vector3(0f, 1.5f, 0f); // Change based on your needs
            subscriptionPanel.transform.rotation = Quaternion.identity; // Facing forward
        }
    }
}
